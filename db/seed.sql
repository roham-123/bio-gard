...
-- Ingredients (multiple rows per name allowed when cost differs per product)
-- Product 1
...
-- Recipe Lines: join on (name, cost_per_kg_gbp) so we get the correct ingredient per recipe
-- Product 1
...
JOIN ingredients i ON i.name = v.ingredient_name AND i.cost_per_kg_gbp = v.cost_per_kg_gbp
...
FROM (VALUES
  ...
  (4, 'Sodium BiCarbonae', 0,    1.20,  6390, 'remainder', 0),
...
FROM (VALUES
  ...
  (4, 'Arthrobacter Globiformis',  1.50e13,  1600.00, 150,  'fixed', 0),
...
-- Seed CFU options (one Default per ingredient; join on id from ingredients that have options)
INSERT INTO ingredient_cfu_options (ingredient_id, label, cfu_per_gram, is_default)
SELECT i.id, 'Default', v.cfu_per_gram, TRUE
FROM (VALUES
  ('Paenibacillus Polymyxa 100B',           0,      1.00e11),
  ('Pseudomonas Putida 500 billion cfu',    1550.00, 1.00e08),
  ('PRO0235',                               71.00,  1.00e11),
  ('Trichoderma Reesi 2E10',                101.54, 2.00e10),
  ('Trichoderma Haziarnim 2E10',            100.00, 2.00e10),
  ('Filler FUN TRICH HARZIANUM FUN 003',    0,      1.00e09),
  ('PRO0235',                               63.38,  4.00e09),
  ('MM3',                                   4.97,   4.00e09),
  ('WW100',                                 62.50,  1.00e11),
  ('Bacillus Polymyxa',                     316.67, 1.00e11),
  ('Pseudomonas Putida',                    1766.67, 5.00e11),
  ('Arthrobacter Globiformis',              1600.00, 1.00e11),
  ('Trichoderma Harzianum 20E9',            58.33,  1.00e10),
  ('Trichoderma Reesi 20E9',                58.33,  1.00e10),
  ('Sugar Multidextrose',                   1.56,   1.00e08),
  ('Sugar',                                 1.56,   1.00e08)
) AS v(ingredient_name, cost_per_kg_gbp, cfu_per_gram)
JOIN ingredients i ON i.name = v.ingredient_name AND i.cost_per_kg_gbp = v.cost_per_kg_gbp
